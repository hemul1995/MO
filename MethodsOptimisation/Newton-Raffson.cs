    ...
    class Newton_Raffson
    {
        ...
        public void _Newton_Raffson(double[] x, double eps = 1e-5)
        {
            ...
            while (k < 10000)
            {
                ...
                //1
                grad.ToList().Select((a, i) => (grad[i] = getDerivate1(x, i))).ToArray();
                //for (int i = 0; i < LENGTH; i++)
                //{
                ...
                //2
                if (Math.Sqrt(grad.Sum(a => a * a)) < eps) break;
                //for (int i = 0; i < LENGTH; i++)
                //{
                //    norm += grad[i] * grad[i];
                //}
                ...
                if (positivOpred(Gesse))
                {
                    ...
                    dk.ToList().Select((a, i) => (dk[i] = -det * grad[i])).ToArray();
                    //for (int i = 0; i < LENGTH; i++)
                    //{
                    //    dk[i] = -det * grad[i];
                    //}
                ...
                {
                    dk.ToList().Select((a, i) => (dk[i] = -grad[i])).ToArray();
                    //for (int i = 0; i < LENGTH; i++)
                    //{
                    //    dk[i] = -grad[i];
                    //}
                ...
                for (int i = 0; i < LENGTH; i++)
                {
                    ...
                    tk[i] = (goldenSection(x, -15, 15, i, 1e-5) - x[i])/dk[i];
                }
                ...
                {
                    x1[i] = x[i] + tk[i] * dk[i];
                    //x1[i] = x[i] + goldenSection(x, -5, 5, i, 1e-5);
